...
| VMware cluster provisioning | Workstation Ansible drives the private node IPs. | Local control-node support plus reachable Ubuntu/Debian guests. Windows host support remains experimental in the roadmap. |
| Terraform VMware provider | Native Go executable loaded from `registry.local/cloudseed/vmdesktop` filesystem mirror. | Go 1.24+ to build; Fusion/Workstation tools to execute; REST listener for network operations. It is not a vSphere provider. |
